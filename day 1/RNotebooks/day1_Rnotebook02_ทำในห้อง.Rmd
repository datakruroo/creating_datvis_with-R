...
ggplot()+
      geom_point(data = dat2007, aes(x = gdpPercap, 
             y = lifeExp,
             col = continent,
             size = pop))
```
...
-   `scale_color_manual(name, values)` ---\> ใช้กับตัวแปรจัดประเภท

-   `scale_color_gradient(name, low, hight)` ---\> ใช้กับตัวแปรเชิงปริมาณ
...
ggplot()+
      geom_point(data = dat2007, aes(x = gdpPercap, 
             y = lifeExp,
             col = continent))+
  scale_color_manual(values = c("red","blue","green","skyblue","orange"))
...
ggplot()+
      geom_point(data = dat2007, aes(x = gdpPercap, 
             y = lifeExp,
             col = pop))+
  scale_color_gradient2(low = "green", mid = "orange",high = "red")
```
...
iris %>% ggplot(aes(x = Sepal.Length,
                    ...
                    col = Species))+
  geom_point()+
  scale_color_manual(values = c("#4D455D" ,
                                "#E96479" ,
                                "#7DB9B6"))
```

**แหล่งอ้างอิงสี ---\> color palette**
...
```{r}
mycolor <- brewer.pal(n=9, name = "Set1")[c(2, 3,5,7,8)]
...
display.brewer.all(type = "qual")
mycolor <- brewer.pal(n=5, name = "Dark2")
ggplot()+
      geom_point(data = dat2007, aes(x = gdpPercap, 
             y = lifeExp,
             col = continent))+
...
ggplot()+
      geom_point(data = dat2007, aes(x = gdpPercap, 
             y = lifeExp,
             col = pop))+
  scale_color_viridis_c(begin = 0.2, end = 0.9, alpha =  0.8,
                        direction = -1,
...
dat2007$gdplow<-dat2007$gdpPercap < 5000
ggplot()+
      geom_point(data = dat2007, aes(x = gdpPercap, 
             y = lifeExp,
             col = gdplow ))+
...
2.  legend ของ size of population

3.  tranparency ของ สี
...
ggplot()+
      geom_point(data = dat2007, aes(x = gdpPercap, 
             ...
             col = continent,
             size = pop))+
  scale_color_brewer(name = "Country Region",type="qual", palette =3)+
  scale_size(name = "Population Size",breaks = c(10^6,10^7, 10^8, 10^9),
...
ggplot()+
      geom_point(data = dat2007, aes(x = gdpPercap, 
             ...
             size = pop),
             alpha = 0.5)+
  scale_color_carto_d(type = "qualitative", palette = 4)+
  scale_size(name = "Population Size",breaks = c(10^6,10^7, 10^8, 10^9),
             labels=c("1","10","100","1000 Millions"))+
...
ggplot()+
  ...
  scale_size(name = "Population Size",breaks = c(10^6,10^7, 10^8, 10^9),
             labels=c("1","10","100","1000 Millions"))+
  scale_shape_manual(values = 20:24)+
  scale_x_log10(breaks = c(250,500,1000,2000,4000,8000,16000,32000,64000))+
  scale_y_continuous(name = "Life Expectancy",limits=c(35,85), breaks = seq(35,85,5))+
  guides(shape = F)+
  theme_classic()
```
...
gapminder %>% 
  ...
  geom_text(aes(label = country), show.legend = F,
            check_overlap = TRUE,
            nudge_x = 0.1)+
  scale_x_log10()
```
...
gapminder %>% 
  ...
  scale_size(name ="Population Size",breaks=c(10^6, 10^7, 10^8, 10^9),
             labels = c("1","10","100","1000 Millions"))+
  scale_color_carto_d(name = "Country Region",type = "qualitative", palette = 1)+
  scale_x_log10(name = "\n Income (gdp per cap ($))")+
