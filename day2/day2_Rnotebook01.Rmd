...
# calcuate percentage
dat <- dat %>%
  mutate(percent1 = item1*100/sum(item1),
         percent2 = item2*100/sum(item2))
```
...
```{r eval = F}
dat %>% 
  gather(item1, item2, key = "item", value = "percent") %>%
  ggplot(aes(x = item, y = percent, fill = level))+
  geom_bar(stat = "identity",position = "fill", alpha=0.8)+
  geom_text(aes(label = percent), 
            position = position_fill(vjust = 0.5))+
  scale_fill_brewer(name = "" ,type="div", palette = "Spectral",
                    direction = 1)+
...
note: ชุดคำสั่งข้างต้นใช้ feature หลายตัวจาก ggrepel รายละเอียดของการกำหนดอาร์กิวเมนท์ต่าง ๆ สามารถศึกษาเพิ่มเติมได้จาก <https://ggrepel.slowkow.com/articles/examples.html>
